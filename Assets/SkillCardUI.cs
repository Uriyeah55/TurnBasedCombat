...
public class SkillCardUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
{
    ...
    void Start()
    {
        ...
        uiBattleManager=manager.GetComponent<UIBattleManager>();
        hideInfoTexts();
    ...
    public void SetSkillData(Skill skill)
    {
        ...
        image.sprite = skill.skillSprite; // Update sprite directly
        currentSkill = skill;
    }
...
public void OnPointerClick(PointerEventData eventData)
{
    ...
    if (GlobalVars.isShuffling)
    {
        ...
        if (emptySlot != null)
        {
            // Check if the skill has already been used
            if (!uiBattleManager.usedSkills.Contains(currentSkill))
            {
                emptySlot.SetSkill(currentSkill);
                emptySlot.gameObject.GetComponent<Image>().sprite = image.sprite;
                uiBattleManager.AddSkillToUsed(currentSkill);
                Debug.Log("Skill placed in shuffle slot: " + currentSkill.skillName);
            }
            else
            {
                Debug.Log("Skill has already been used.");
            }
        }
        else
        {
